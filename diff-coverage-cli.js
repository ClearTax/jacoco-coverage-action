...
const { calculateDiffLineCoverage } = require('./diff-line-coverage');
const fs = require('fs');
...
function printHelp() {
  console.log(`
Usage: node diff-coverage-cli.js [options]
...
function formatAsText(result) {
  ...
  for (const [filePath, fileResult] of Object.entries(result.fileDetails)) {
    ...
    output += `  Coverage Percentage: ${fileResult.coveredChangedLines / fileResult.totalChangedLines * 100}%\n`;
    output += `  Line Details:\n`;
...
async function main() {
  try {
    ...
    const result = await calculateDiffLineCoverage(jacocoXmlPath, baseRef);
